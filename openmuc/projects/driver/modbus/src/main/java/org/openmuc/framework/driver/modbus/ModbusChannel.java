...
public class ModbusChannel {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ModbusChannel.class);
    ...
    private void setFunctionCode() {
        if (accessFlag.equals(EAccess.READ)) {
            selectFunctionCodeForReading();
        }
        else {
            selectFunctionCodeForWriting();
        }
    }
     ...
     * @throws Exception
     */
    private void selectFunctionCodeForReading() {

        switch (datatype) {
        case BOOLEAN:
            setReadFunctionCodeIfCorrect(EPrimaryTable.COILS, EFunctionCode.FC_01_READ_COILS,
                    EPrimaryTable.DISCRETE_INPUTS, EFunctionCode.FC_02_READ_DISCRETE_INPUTS);
            break;
        case SHORT:
        case INT16:
        case INT32:
        ...
        case LONG:
        case BYTEARRAY:
            setReadFunctionCodeIfCorrect(EPrimaryTable.HOLDING_REGISTERS, EFunctionCode.FC_03_READ_HOLDING_REGISTERS,
                    EPrimaryTable.INPUT_REGISTERS, EFunctionCode.FC_04_READ_INPUT_REGISTERS);
            break;
        default:
    ...
    private void setReadFunctionCodeIfCorrect(EPrimaryTable ePrimaryTable1, EFunctionCode eFunctionCode1,
            EPrimaryTable ePrimaryTable2, EFunctionCode eFunctionCode2) {
        if (primaryTable.equals(ePrimaryTable1)) {
            functionCode = eFunctionCode1;
        }
        else if (primaryTable.equals(ePrimaryTable2)) {
            functionCode = eFunctionCode2;
        }
        else {
    ...
    private void selectFunctionCodeForWriting() {
        switch (datatype) {
        case BOOLEAN:
            setWriteFuctionCodeIfCorrect(EPrimaryTable.COILS, EFunctionCode.FC_05_WRITE_SINGLE_COIL);
            break;
        case SHORT:
        case INT16:
        case UINT16:
            setWriteFuctionCodeIfCorrect(EPrimaryTable.HOLDING_REGISTERS, EFunctionCode.FC_06_WRITE_SINGLE_REGISTER);
            break;
        case INT32:
        case UINT32:
        case FLOAT:
        ...
        case LONG:
        case BYTEARRAY:
            setWriteFuctionCodeIfCorrect(EPrimaryTable.HOLDING_REGISTERS, EFunctionCode.FC_16_WRITE_MULTIPLE_REGISTERS);
            break;
        default:
    ...
    private void setWriteFuctionCodeIfCorrect(EPrimaryTable ePrimaryTable, EFunctionCode eFunctionCode) {
        if (primaryTable.equals(ePrimaryTable)) {
            functionCode = eFunctionCode;
        }
        else {
