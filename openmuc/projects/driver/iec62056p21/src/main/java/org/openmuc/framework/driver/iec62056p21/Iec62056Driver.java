...
public final class Iec62056Driver implements DriverService {
    ...
    private static final String CUSTOM_ACKNOWLEDGE_MODE = "-cam";
    private static final String SELECT_EXPECT_DATA = "-sed";
    private static final String SELECT_MANUFACTURER_DATA = "-smd";
    ...
    private char customAcknowledgeMode = '0';
    private final String selectExpectData = null;
    ...
    private static final String SETTINGS = "[" + BAUD_RATE_CHANGE_DELAY + " <baud_rate_change_delay>] [" + TIMEOUT_PARAM
            ...
            + " is set\n" + CUSTOM_ACKNOWLEDGE_MODE
            + "<character_of_custom_acknowledge_mode> Sets a custom  acknowledge mode.\n" + SELECT_EXPECT_DATA
            + "<select_expected_data_string>\n" + SELECT_MANUFACTURER_DATA + "<select_manufacture_data_string>\n";
    ...
    private Builder getConfiguredBuilder() {
        return new Iec21Port.Builder(serialPortName).setBaudRateChangeDelay(baudRateChangeDelay)
                ...
                .setSelectManufacturerData(selectManufacturerData)
                .setSelectExpectData(selectExpectData)
                .setAcknowledgeMode(customAcknowledgeMode);
    }
    ...
    private void parseArguments(String[] args, boolean isScan) throws ArgumentSyntaxException {
        ...
        for (; i < args.length; i++) {
            ...
            else if (args[i].equals(CUSTOM_ACKNOWLEDGE_MODE)) {
                ++i;
                customAcknowledgeMode = getCharValue(args, i, CUSTOM_ACKNOWLEDGE_MODE);
            }
            else if (args[i].equals(SELECT_MANUFACTURER_DATA)) {
                ++i;
                selectManufacturerData = getStringValue(args, i, SELECT_MANUFACTURER_DATA);
            }
            else if (args[i].equals(SELECT_EXPECT_DATA)) {
                ++i;
                selectManufacturerData = getStringValue(args, i, SELECT_EXPECT_DATA);
            }
            else {
    ...
    private char getCharValue(String[] args, int i, String parameter) throws ArgumentSyntaxException {
        char ret;
        checkParameter(args, i, parameter);
        try {
            ret = parameter.charAt(0);
        } catch (IndexOutOfBoundsException e) {
            throw new ArgumentSyntaxException("Missing char value after parameter " + parameter);
        }
        return ret;
    }
